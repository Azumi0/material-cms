...
class PagerWidget extends Widget
{
    ...
    public $disableOnFirstLast = true;

    protected $shCurrentPage = null;
    protected $shPageCount = null;
    /**
    ...
    protected function renderPageButtons()
    {
        ...
        if ($this->prevPageLabel !== false) {
            ...
            if ($this->disableOnFirstLast){
                $disabler = $currentPage <= 0;
            } else {
                $disabler = false;
            }
            $buttons[] = $this->renderPageButton($this->prevPageLabel, $page, $this->prevPageCssClass, $disabler, false);
        }
        ...
        if ($this->nextPageLabel !== false) {
            ...
            if ($this->disableOnFirstLast){
                $disabler = $currentPage >= $pageCount - 1;
            } else {
                $disabler = false;
            }
            $buttons[] = $this->renderPageButton($this->nextPageLabel, $page, $this->nextPageCssClass, $disabler, false);
        }
        ...
        $temp = Html::tag('ul', implode("\n", $buttons), $this->options);

        return Html::tag('nav', $temp, ['aria-label' => 'Page navigation']);
    }
    ...
    protected function renderPageButton($label, $page, $class, $disabled, $active)
    {
        ...
        if ($this->shCurrentPage == 0 && $page == 0) {
            if (isset($linkOptions['class'])) {
                $linkOptions['class'] .= ' firstPage';
            } else {
                $linkOptions['class'] = 'firstPage';
            }
        }

        if ((($this->shPageCount - 1) == $page) && ($this->shCurrentPage == $page)){
            if (isset($linkOptions['class'])) {
                $linkOptions['class'] .= ' lastPage';
            } else {
                $linkOptions['class'] = 'lastPage';
            }
        }
